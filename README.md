...
6. Go into `./ai` folder
7. `export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512` (on Linux) or `set PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:512` (on Windows)
8. `python prepare.py`
9. Go into `train.py` and configure which pre-trained model you want to use.
10. `python train.py`
